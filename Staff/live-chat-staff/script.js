...
const chatSessions = [
    {
        ...
        messages: [
            ...
            { sender: 'staff', name: 'Mushfikur', text: 'I apologise for the inconvenience. Let me look into that right away.', time: '09:33' },
        ]
    },
    {
...
function renderInbox() {
    ...
    let d = filterVal ? chatSessions.filter(c => c.status === filterVal) : [...chatSessions];
    if (sortVal === 'newest') d.sort((a, b) => b.startedAt.localeCompare(a.startedAt));
    if (sortVal === 'oldest') d.sort((a, b) => a.startedAt.localeCompare(b.startedAt));
    ...
    document.querySelectorAll('.btn-open-chat').forEach(btn => btn.addEventListener('click', () => openChatroom(btn.dataset.id)));
}
...
function openChatroom(id) {
    ...
    document.getElementById('chatroomChatId').textContent = `Chat Room – ${chat.id}`;
...
function sendMessage() {
    ...
    input.value = ''; renderMessages(chat);
}
...
document.getElementById('endConvoBtn').addEventListener('click', () => {
    ...
    chat.status = 'Closed';
    const now = new Date();
    chat.endedAt = `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, '0')}-${String(now.getDate()).padStart(2, '0')} ${String(now.getHours()).padStart(2, '0')}:${String(now.getMinutes()).padStart(2, '0')}`;
